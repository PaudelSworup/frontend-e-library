...
const ReqestThumbNail = ({ result }) => {
  ...
  const download = (bookid)=>{
    downloadBook(bookid).then((res)=>{
      console.log(res)
    }).catch((err)=>console.log(err))
  }
  return (
    <div className="flex gap-3 ml-2 mb-2 h-[270px] main_div_width  mx-auto md:w-[330px]  p-3 rounded-md bg-[#252525]">
      ...
      <div className="flex flex-col  gap-4 text-white">
        ...
        <div className="rounded-3xl flex flex-col justify-center items-center mt-auto text-center tracking-widest  p-2 bg-[#6b6b6b]">
          <div className="flex">
            {result && result.issueStatus ===1 && <div className="flex items-center px-3"><FaDownload  onClick={()=>download(result?.books_id?._id)} /></div>}
            {result?.issueStatus === 1
              ? 
              "Approved"
